...
from featurestore.lineage import Lineage, LineageClosure
...
from featurestore.snapshot import ReproducibilityCheck, TrainingSetSnapshot
from featurestore.snapshot import check_reproducible as _check_reproducible
from featurestore.snapshot import snapshot_training_set as _snapshot_training_set
...
class FeatureStore:
    ...
    def lineage(self, feature_name: str, version: int | None = None) -> Lineage:
        """The direct lineage of one feature version, defaulting to its
        latest version: its source table and the upstream features named in
        its ``depends_on``. See :mod:`featurestore.lineage`.
        """
        return self.registry.lineage(feature_name, version)

    def lineage_closure(self, feature_name: str, version: int | None = None) -> LineageClosure:
        ...
        """
        return self.registry.lineage_closure(feature_name, version)
    ...
    def snapshot_training_set(
        ...
        *,
        ...
        """Freeze a point-in-time join of ``entity_rows`` against
        ``feature_names`` into a reproducible, content-addressed training
        set, plus a manifest recording every feature version and source it
        rests on. See :mod:`featurestore.snapshot`.
        """
        return _snapshot_training_set(
            self.registry,
            entity_rows,
            feature_names,
    ...
    def check_reproducible(self, snapshot: TrainingSetSnapshot) -> ReproducibilityCheck:
        """Whether ``snapshot`` can still be reproduced from this store's
        current state, and if not, exactly which (row, feature) value
        changed and how. See :mod:`featurestore.snapshot`.
        """
        return _check_reproducible(self.registry, snapshot)
