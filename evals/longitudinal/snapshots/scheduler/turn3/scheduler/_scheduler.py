...
class Run:
    ...
    def _finalize_failed(self, name: str, exc: BaseException) -> None:
        ...
        self._record(name, EventType.FAILED, attempt=result.attempts, error=exc)
    ...
    def _finalize_skipped(self, name: str) -> None:
        ...
        self._remaining_count -= 1
        self._ready_since.pop(name, None)
        self._record(name, EventType.SKIPPED, attempt=self._attempts.get(name, 0))
    ...
    def _finish(self) -> None:
        ...
        depends_on = {name: task.depends_on for name, task in self._tasks.items()}
        summary = build_summary(depends_on, self._results, resource_usage)
    ...
    def _record(
        ...
        **extra: Any,
        ...
        """Append a structured event for ``name`` to the in-memory log and,
        if this run was given a real journal, to disk. Must be called with
        ``self._lock`` held, so ``seq`` reflects the true order of
        transitions."""
        event = Event(
            seq=self._next_seq,
            time=time.time(),
