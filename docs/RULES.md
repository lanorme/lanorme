...
it is not under a tests directory, because pytest would collect it. LaNorme
pays that cost itself: its `test_coverage.py` and `test_style.py` checks are
production modules the `TYPE` rules no longer see.
...
names that are not valid identifiers (cannot be written as `x.name`);
`hasattr` on a receiver bound by a plain `import` (`hasattr(os, "fork")`,
...
`label: word = value` is a labelled note unless the word is a type: a
builtin, a name that is not all lowercase, or a lowercase name the module
imports (`# TODO: retries = 5` and `# cython: boundscheck=False` are notes,
`# x: int = 5` is code, and so is `# created: datetime = now()` under
`from datetime import datetime`); an assignment of a foreign literal
(`# enabled = true`, `# timeout = null`) is TOML, YAML or JSON, not Python; a
keyword followed only by an adverb (`# return early`, `# import lazily`,
`# raise instead`) is a prose fragment, unless the adverb is an `-ly` word
the module binds itself (`# return quickly` where `quickly` is assigned is
code); the lines that follow a `Usage:`, `Example:` or `e.g.:`
...
Measured against the 227-comment corpus under
`evals/corpora/comments_commented_code/` with `evals/score_cmt001.py`:
**P = 1.000 / R = 1.000 / F1 = 1.000** (TP = 96, FP = 0, FN = 0, TN = 131),
the same on the dev split, the holdout split and the 40 generated holdout
comments.
...
`all(...)`, is a different operation, not a renamed variable. A call to a name
the function binds itself (a parameter, a local variable, a nested definition)
is a call through a variable, so that name is abstracted with the variable;
...
(the misses are the reordered pairs, by design).
...
what they do is two functions: an operator flipped (`<` against `>`, `and`
against `or`), a called name changed (`min` against `max`, `.append` against
`.remove`), a statement moved into a new branch. So is a pair that differs in
...
few inserted, removed or reordered statements; `op_jaccard`, `call_jaccard`
and `str_jaccard`, the share of the smaller side's operations, called names
and strings that the other side also carries, where 1.0 means nothing changed
and things were only added; and `attr_jaccard`, a weighted Jaccard over
attribute names that rejects the near-disjoint attribute sets of parallel
mappers over different source objects. The `*_jaccard` keys keep their names;
the measure behind the first three is that containment share, which unlike a
Jaccard does not punish drift. Equality/dunder/`@property` boilerplate never
flags.

Known misses, by design: a call renamed to a synonym (`save` against
`persist`) is indistinguishable from a call changed to its opposite, so called
names must agree; a pair that differs only in its error messages; a pair whose
attribute sets are fully renamed (`x`/`y` against `lon`/`lat`).

The thresholds are derived on the dev split of the bundled corpus
(`evals/corpora/duplication_similar/`, scorer `evals/score_similar.py`) and
never on its holdout split. Measured on the holdout split: **precision 1.000 /
recall 0.966 / F1 0.982**; on the generated holdout cases alone, precision
1.000 / recall 1.000; on the dev split, precision 1.000 / recall 0.800.
...
that happens to be named `override` (Django's `@translation.override("fr")`)
does not exempt a method.
...
- A camelCase method on a class that may inherit the name
  (`mousePressEvent` on a `QWidget` subclass, `dataReceived`, `onMessage`):
  PEP 8 allows mixedCase only where it is the prevailing style, so the name
  is the base API's. A class may inherit a name through an external base (one
  the module does not define), or through a base the module defines that has
  an external base itself or defines the same method. `object`, `ABC`,
  `Generic`, `Protocol`, `NamedTuple`, `TypedDict` and the `Enum` family
  (`Enum`, `IntEnum`, `StrEnum`, `Flag`, `IntFlag`) carry no method names and
  do not count. The same name on any other class, or at module level, is
  judged.
...
not reported. A method on a class that may inherit its name, by the same
test as the camelCase exemption above, may be overriding it (`do_GET` on a
request handler, `process` on a SQLAlchemy type), so it is not reported
either; nor is any name `NAMING-007` leaves alone as
...
`subprocess.run`. A name counts as shadowed, and so as unknown, only when
the rebinding (a parameter, a local `def`, an assignment) is visible at the
call under Python's scoping: at module level, or in the calling function or
a function around it. So a local `run(cmd, shell=True)` never fires, while a
method named `exec` leaves a module-level `exec(code)` reported (a method
name never shadows a global) and another function's parameter named `sp`
does not hide `import subprocess as sp`. A `shell=` / `verify=` value that is
not a literal is ambiguous and stays quiet; `usedforsecurity=` is the
exception, since only a literal `False` declares a non-security use.
...
- `CRYPTO-001`: `hashlib.md5` / `hashlib.sha1` / `hashlib.new("md5"/"sha1",
  ...)` used for security (a literal `usedforsecurity=False` is honoured on
  all three; a value that is not a literal fires, since it may be `True`),
  `ssl.PROTOCOL_SSLv2` / `SSLv3` / `TLSv1` / `TLSv1_1` referenced
...
- `AUTHN-001`: default-on. `@router.post` / `put` / `patch` / `delete`
  handlers must have an auth dependency: a `Depends(...)` or `Security(...)`
  whose declared dependency (its first argument, or `dependency=`) is named
  exactly `get_current_user` or starts with `require_`. It counts in a
  parameter annotation (`Annotated[User, Depends(get_current_user)]`), as the
  parameter default itself (`user: User = Depends(get_current_user)`), or in
  the decorator's `dependencies=[...]` list. `get_current_user_optional`, an
  auth name passed as some other argument, and a marker nested inside another
  call in a default do not count. FastAPI-shaped;
...
- `SQL-001`: default-on. AST-based: only flags SQL string literals that
  ...
  `read_sql_query`). Unwraps `text(...)` constructors, resolves a bare name
  only to a module-level string constant or a string assigned in the same
  function (never to a parameter, so a parameter named like a constant
  elsewhere in the file stays unknown), and treats `%`-formatted /
