...
Every check that treats test code differently goes through these predicates,
so their boundaries are pinned here once. The per-check behaviour (a boundary
file exempt from a production rule, or selected by a test rule) is covered in
``test_test_file_boundaries.py``.
"""
...
def test_module_rejects_support_and_lookalike_names(relative: str) -> None:
    ...
    # Assert: only the collected stem shapes count, not conftest, a "tests" module or a lookalike.
    assert not matched
...
@pytest.mark.parametrize(
    "relative",
    ["factories/user.py", "pkg/fixtures/data.py", "tests/helpers.py", "pkg/contest.py"],
)
def test_support_rejects_fixtures_outside_tests_and_plain_helpers(relative: str) -> None:
    # Act
    matched = is_test_support(relative)

    # Assert: a production factories/ package is not test scaffolding, nor is a conftest lookalike.
    assert not matched
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("src/pkg/tests/fixtures/db.py", True),
        ("tests", False),
        ("pkg/tests.py", False),
        ("pkg/tests_utils.py", False),
...
def test_under_tests_dir_matches_a_directory_part_only(relative: str, expected: bool) -> None:
    ...
    # Assert: the directory name must be exactly tests or test; a file or directory
    # merely containing the word does not count.
...
@pytest.mark.parametrize(
    "relative",
    ["conftest.py", "test/main.py", "src/pkg/tests/fixtures/db.py", "tests/helpers.py"],
)
def test_test_file_accepts_root_level_and_nested_boundary_shapes(relative: str) -> None:
    # Act
    matched = is_test_file(relative)

    # Assert: a root conftest, a singular test/ directory and a nested fixtures package are all tests.
    assert matched


@pytest.mark.parametrize("relative", ["pkg/tests_utils.py", "pkg/contest.py", "factories/user.py"])
def test_test_file_rejects_lookalikes(relative: str) -> None:
    # Act
    matched = is_test_file(relative)

    # Assert: a name that only contains "test" is production code.
    assert not matched
