...
Both kinds share the three predicates below so a file cannot be a test for
one rule and production code for another.
...
- :func:`is_test_file`: the union of the three, the definition the exempting
  checks use. The measurement behind it is recorded under "Test files" in
  ``docs/RULES.md``.
"""
...
def _as_posix(relative: str | Path) -> PurePosixPath:
...
def is_under_tests_dir(relative: str | Path) -> bool:
    ...
    parts = _as_posix(relative).parts
...
def is_test_support(relative: str | Path) -> bool:
    ...
    """
    path = _as_posix(relative)
    if path.name in TEST_SUPPORT_FILENAMES:
        return True
    if not is_under_tests_dir(path):
        return False
    return path.stem in TEST_SUPPORT_NAMES or any(part in TEST_SUPPORT_NAMES for part in path.parts[:-1])
