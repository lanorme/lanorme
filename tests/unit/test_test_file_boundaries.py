...
_LONG_SPAN = (
    ...
    "    rc = 0\n"
    "    total = 0\n" + "".join(f"    total += {i} - {i}\n" for i in range(40)) + "    return rc + total\n"
)


def _named_args() -> NamedArgsCheck:
    check = NamedArgsCheck()
    check.enabled = True
    return check


def _configured(check, settings: dict):
...
PRODUCTION_RULES = {
    "named_args": (_named_args, "def f(a, b):\n    return a\n"),
    "duplication": (DuplicationCheck, _DUP_PAIR),
    "file_limits": (FileLimitsCheck, "def f(a, b, c, d, e, g, h, i, j):\n    return a\n"),
    "similarity": (lambda: _configured(SimilarityCheck(), {"enabled": True}), _CLONE_PAIR),
    ...
    "naming_scope": (lambda: _configured(NamingScopeCheck(), {"enabled": True}), _LONG_SPAN),
    "strong_types": (
        StrongTypesCheck,
        "from typing import Any\n\n\ndef f(x: dict[str, Any]) -> None:\n    return None\n",
    ),
    "stale_paths": (
        lambda: _configured(StalePathsCheck(), {"tokens": ["old_pkg/"]}),
        "# moved from old_pkg/legacy.py\nX = 1\n",
    ),
    "attribute_access": (
        lambda: _configured(AttributeAccessCheck(), {"enabled": True}),
        "def f(x):\n    return hasattr(x, 'foo')\n",
    ),
    "docstrings": (
        lambda: _configured(DocstringsCheck(), {"enabled": True}),
...
def _flagged_files(result) -> set[str]:
...
def test_test_style_judges_only_the_collected_module(tmp_path: Path) -> None:
    ...
    check = StyleCheck(enabled=True)

    # Act
    result = check.run(src_root=str(tmp_path))

    # Assert: helpers, conftest and fixtures are never judged as tests, even under tests/.
    assert _flagged_files(result) == {"pkg/test_module.py"}


def _coverage_run(tmp_path: Path, *, partner: str, body: str) -> list:
...
@pytest.mark.parametrize(
    "partner",
    ["tests/integration/helpers.py", "tests/integration/conftest.py", "tests/integration/fixtures/users.py"],
)
def test_coverage_does_not_credit_support_files_as_partners(tmp_path: Path, partner: str) -> None:
    ...
    hits = _coverage_run(tmp_path, partner=partner, body="from endpoints.users import f\n")
...
@pytest.mark.parametrize(
    "partner", ["tests/integration/api/test_users.py", "tests/integration/users_test.py"]
)
def test_coverage_credits_collected_modules_anywhere_under_the_root(tmp_path: Path, partner: str) -> None:
    # Arrange / Act: the partner is a nested test module, or a *_test.py module, under the root.
    hits = _coverage_run(tmp_path, partner=partner, body="def test_f(): ...\n")
