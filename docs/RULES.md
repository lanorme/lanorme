...
handled specially. Known recall gaps: fully renamed attribute sets and
error-message-only drift.

Measured against `evals/corpora/duplication_similar/` with
`evals/score_similar.py`:

| split | P | R | F1 | TP | FP | FN | TN |
| --- | --- | --- | --- | --- | --- | --- | --- |
| dev | 1.000 | 0.800 | 0.889 | 20 | 0 | 5 | 34 |
| holdout | 0.519 | 0.692 | 0.593 | 27 | 25 | 12 | 19 |
| holdout, generated only | 0.324 | 0.522 | 0.400 | 12 | 25 | 11 | 3 |

The holdout number is the honest one: the dev files were visible while the
thresholds were tuned, and the generated cases (label-breaking edits of
CPython functions) find the false positives dev never showed, so a
