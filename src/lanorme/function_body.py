...
def _iter_own_scope(*, func: FunctionNode) -> Iterator[ast.AST]:
    ...
    loses no binding.
    """
    # ``iter_child_nodes`` covers the defaults and skips the ``None`` a
    # keyword-only parameter without a default leaves in ``kw_defaults``.
    pending: list[ast.AST] = [func.args, *func.body]
    while pending:
        node = pending.pop()
        yield node
